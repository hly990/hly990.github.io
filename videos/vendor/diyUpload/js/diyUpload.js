    ...
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			if(opt.imageString){
				var imageString = opt.imageString;
				delete opt.imageString;
			}
			if( opt.error ) {
			...
			webUploader.on('uploadSuccess',function( file, response ){
				...
				$diyBar.fadeOut( 1000 ,function(){
					...
					$fileBox.hover(function(){
						    $(this).children('.diySuccess').hide();
						    $(this).children('.diyCancel').show();
						},function(){
						    $(this).children('.diySuccess').show();
				...
				if ( successCallBack ) {
					response.fileId='#fileBox_'+file.id;
			...
			webUploader.on('uploadStart',function( file ){
				var $fileBox = $('#fileBox_'+file.id);
				if(webUploader.getStats().successNum-webUploader.getStats().cancelNum>=opt.fileNumLimit){
					var err = 'upload failed!  The number of uploaded files exceeds the limit!';
				    //if ( errorCallBack ) {
					//    errorCallBack( err );
				    //}
				    alert(err);
				    removeLi( $fileBox, file.id, webUploader );	
			...
			webUploader.on('uploadError',function( file, reason ){
				...
				var err = 'upload failed! Document:'+file.name+' error code:'+reason;
				if ( errorCallBack ) {
					errorCallBack( err );
				}
			});
			...
			webUploader.on('error', function( code ) {
				...
				switch( code ) {
					...
					break;
					case  'Q_EXCEED_NUM_LIMIT' : text = 'The number of uploaded files exceeds the limit!' ;
					break;
					case  'F_EXCEED_SIZE' : text = 'The file size exceeds the limit!';
					break;
					case  'Q_EXCEED_SIZE_LIMIT' : text = 'The total size of all files exceeds the limit!';
					break;
					case 'Q_TYPE_DENIED' : text = 'The file type is incorrect or is empty!';
					break;
					default : text = 'unknown mistake!';
 					break;	
				}
            	alert( text );
        	});
	...
	function getOption(objId) {
		...
		*/
		return {
			...
			pick:{
				id:objId,
				label:"Click to select a picture"
			},
			//类型限制;
			accept:{
				title:"Images",
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {
			...
			var uploadStart = function (){
				webUploader.upload();
				$startButton.text('Pause upload').one('click',function(){
						webUploader.stop();
						$(this).text('Continue uploading').one('click',function(){
								uploadStart();
						});
				});
			}
